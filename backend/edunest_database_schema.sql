...
CREATE TABLE otps (
  ...
  shipment_id BIGINT UNSIGNED NULL,                    -- added: set only for purpose='delivery_confirmation'; FK added in DEFERRED FOREIGN KEYS block (shipments is created later)
  identifier  VARCHAR(190) NOT NULL,                    -- email or phone (pre-signup OTP too)
  otp_hash    VARCHAR(255) NOT NULL,
  purpose     ENUM('login','signup','password_reset','phone_verify','email_verify','transaction','delivery_confirmation') NOT NULL,
  expires_at  TIMESTAMP NOT NULL,
  consumed_at TIMESTAMP NULL,
  attempt_count TINYINT UNSIGNED NOT NULL DEFAULT 0,
  created_at  TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_otps_identifier (identifier, purpose),
  KEY idx_otps_shipment (shipment_id),
...
CREATE TABLE inventory (
  ...
  warehouse_id  BIGINT UNSIGNED NULL,                      -- added: optional link to a normalized `warehouses` row; warehouse_location (free text) is kept for backward compatibility
  ...
  CONSTRAINT fk_inventory_warehouse FOREIGN KEY (warehouse_id) REFERENCES warehouses(id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB;
...
-- Warehouses: a normalized location entity. Deliberately additive alongside the existing
-- inventory.warehouse_location free-text column rather than replacing it — existing
-- checkout/stock-reservation code keys off warehouse_location + dealer_id today, and
-- rewriting that in the same pass as adding the entity would risk breaking a working
-- feature. inventory.warehouse_id (added below, nullable) lets inventory rows optionally
-- point at a real Warehouse row going forward without forcing a backfill or breaking any
-- row that only has warehouse_location set.
...
-- Quantity-change-request workflow: every stock quantity edit that isn't a normal
-- sale/return/purchase (i.e. a manual correction someone is requesting) goes through here
-- and only touches inventory.quantity_available once approved — see
-- quantityChangeRequest.service.ts's approve() for the transactional update + the
-- stock_history 'adjustment' row it writes at the same time.
CREATE TABLE quantity_change_requests (
  id                  BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  uuid                CHAR(36) NOT NULL DEFAULT (UUID()),
  inventory_id        BIGINT UNSIGNED NOT NULL,
  current_quantity    INT NOT NULL,                 -- snapshot of quantity_available at request time, for audit even if it drifts before approval
...
CREATE TABLE orders (
  ...
  dealer_total_amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
  margin_amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
  platform_fee_amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
  net_revenue_amount DECIMAL(12,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE order_items (
  ...
  dealer_unit_price DECIMAL(12,2) NOT NULL DEFAULT 0.00,
  tax_amount    DECIMAL(12,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE production_checkpoints (
  ...
  updated_by_type        ENUM('dealer','admin','system') NOT NULL DEFAULT 'system',
  created_at             TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE invoices (
  ...
  cgst_amount   DECIMAL(12,2) NOT NULL DEFAULT 0,          -- added: persisted GST breakdown, not just the flat tax_amount (was previously computed only for the PDF and discarded)
  sgst_amount   DECIMAL(12,2) NOT NULL DEFAULT 0,
  utgst_amount  DECIMAL(12,2) NOT NULL DEFAULT 0,
  igst_amount   DECIMAL(12,2) NOT NULL DEFAULT 0,
  is_inter_state TINYINT(1) NOT NULL DEFAULT 0,
  place_of_supply VARCHAR(100) NULL,                       -- billing state at the time this invoice was issued
  seller_gstin  VARCHAR(15) NULL,                           -- snapshot of the company's GSTIN at issue time — stays correct even if Settings changes later
  buyer_gstin   VARCHAR(15) NULL,                           -- snapshot of the school's GSTIN at issue time
...
-- E-way bills: record-keeping only. This project has NO integration with the government
-- e-way bill / IRP portal — there is no API call anywhere that generates a real e-way bill
-- number. eway_bill_number is deliberately nullable and is filled in by an admin pasting
-- the number they generated on the government portal, exactly like the existing
-- `gstin` fields are self-reported, not government-verified. Do not read a non-null
-- eway_bill_number as proof of a genuine government-issued bill without checking who/how it
-- was entered — see eway.service.ts's comment for the same caveat in code.
CREATE TABLE eway_bills (
  id                BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  uuid              CHAR(36) NOT NULL DEFAULT (UUID()),
  order_id          BIGINT UNSIGNED NOT NULL,
  invoice_id        BIGINT UNSIGNED NULL,
  eway_bill_number  VARCHAR(20) NULL,                      -- manually entered after generating on the government e-way bill portal; NULL until then
...
CREATE TABLE shipments (
  ...
  delivery_partner_id BIGINT UNSIGNED NULL,          -- added: assigned delivery partner (nullable — carrier-only shipments keep working)
  status        ENUM('pending','ready_for_dispatch','assigned','picked_up','dispatched','in_transit','out_for_delivery','delivery_attempted','rescheduled','delivered','failed') NOT NULL DEFAULT 'pending',
  recipient_name  VARCHAR(150) NULL,                 -- added: who actually received the delivery (OTP verification target)
  recipient_phone VARCHAR(20) NULL,
  alternate_recipient_approved_by BIGINT UNSIGNED NULL, -- added: Super Admin who approved delivery to someone other than the school contact
  shipped_at    TIMESTAMP NULL,
  delivered_at  TIMESTAMP NULL,
  created_at    TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_shipments_order (order_id),
  KEY idx_shipments_tracking (tracking_number),
  KEY idx_shipments_deliverypartner (delivery_partner_id, status),
  CONSTRAINT fk_shipments_order FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_shipments_deliverypartner FOREIGN KEY (delivery_partner_id) REFERENCES delivery_partners(id) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT fk_shipments_altrecipient_approver FOREIGN KEY (alternate_recipient_approved_by) REFERENCES users(id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB;
...
-- Append-only delivery status audit trail (Section 5: every status change must record
-- order/shipment id, previous status, new status, who changed it, when, and notes). This
-- widens the original delivery_updates table rather than adding a parallel one, since it
-- was already the shipment-events table and every existing row stays valid (new columns
-- are all nullable).
CREATE TABLE delivery_updates (
  id                BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  shipment_id       BIGINT UNSIGNED NOT NULL,
  order_id          BIGINT UNSIGNED NULL,             -- added: denormalized for fast order-level history queries
  update_type       ENUM('note','reschedule','failed_attempt','delivered') NOT NULL,
  previous_status   VARCHAR(30) NULL,                 -- added
  new_status        VARCHAR(30) NULL,                 -- added
  updated_by        BIGINT UNSIGNED NULL,             -- added: admin/staff user who made the change
  updated_by_type   ENUM('admin','staff','delivery_partner','system') NOT NULL DEFAULT 'system', -- added
  new_delivery_date DATE NULL,                        -- added: set when update_type = 'reschedule'
  message           VARCHAR(255) NULL,
  created_at        TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_deliveryupdates_shipment (shipment_id),
  KEY idx_deliveryupdates_order (order_id, created_at),
  ...
  CONSTRAINT fk_deliveryupdates_order FOREIGN KEY (order_id) REFERENCES orders(id) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_deliveryupdates_user FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE quotation_request_products (
  ...
  product_id    BIGINT UNSIGNED NULL,
  kit_id        BIGINT UNSIGNED NULL,
  custom_item_name VARCHAR(220) NULL,
  custom_item_description VARCHAR(255) NULL,
  custom_item_school_price DECIMAL(12,2) NULL,
  custom_item_dealer_price DECIMAL(12,2) NULL,
  custom_item_image_file_id BIGINT UNSIGNED NULL,
  custom_item_external_url VARCHAR(500) NULL,
  custom_item_notes TEXT NULL,               -- for items not in catalog
  quantity      INT UNSIGNED NOT NULL DEFAULT 1,
...
CREATE TABLE dealer_quotation_items (
  ...
  school_unit_price DECIMAL(12,2) NULL,
  dealer_unit_price DECIMAL(12,2) NOT NULL DEFAULT 0.00,
...
ALTER TABLE quotation_request_products ADD CONSTRAINT fk_quotreqprod_image FOREIGN KEY (custom_item_image_file_id) REFERENCES uploaded_files(id) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE otps                ADD CONSTRAINT fk_otps_shipment            FOREIGN KEY (shipment_id) REFERENCES shipments(id) ON DELETE CASCADE ON UPDATE CASCADE;
