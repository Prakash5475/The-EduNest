...
import { invoiceEmailTemplate } from '@/emails/templates/invoice.template';
...
export class EmailService {
  ...
  async sendInvoiceEmail(to: string, schoolName: string, invoiceNumber: string, totalAmount: string, downloadUrl: string): Promise<void> {
    const { subject, html } = invoiceEmailTemplate(schoolName, invoiceNumber, totalAmount, downloadUrl);
    await emailQueue.add(EMAIL_JOB_NAMES.INVOICE, { to, subject, html });
  }
}
