...
import { calculateGstBreakdown, getCompanyGstProfile } from '@/helpers/gst.helper';
import { uploadBufferToLocal } from '@/helpers/localBufferUpload.helper';
...
export class PaymentService {
  ...
  private async generateInvoice(orderId: bigint, paymentType: AmountType | 'refund', isFullyPaid: boolean) {
    ...
    // Computed BEFORE the invoice row is created (not just for the PDF afterward) so the
    // breakdown is actually persisted — this used to be computed only for rendering and
    // discarded, meaning GST reports had to re-derive it from the flat taxAmount instead of
    // reading what was really invoiced.
    const supplyState = order.schoolAddress?.state ?? '';
    const gst = await calculateGstBreakdown(Number(order.taxAmount), supplyState);
    const companyGstProfile = await getCompanyGstProfile();
    ...
    const invoice = await prisma.invoice.create({
      data: {
        ...
        cgstAmount: gst.cgst,
        sgstAmount: gst.sgst,
        utgstAmount: gst.utgst,
        igstAmount: gst.igst,
        isInterState: gst.isInterState,
        placeOfSupply: supplyState || null,
        sellerGstin: companyGstProfile.gstin || null,
        buyerGstin: order.school.gstin || null,
    ...
    try {
      const pdfBuffer = await renderInvoicePdf({
        ...
        companyGstin: companyGstProfile.gstin || null,
        companyLegalName: companyGstProfile.legalName || null,
        companyTradeName: companyGstProfile.tradeName || null,
        companyAddress: [companyGstProfile.addressLine1, companyGstProfile.addressLine2, companyGstProfile.locality, `${companyGstProfile.city}, ${companyGstProfile.state} ${companyGstProfile.pincode}`].filter(Boolean).join(', ') || null,
        ...
        isInterStateSupply: gst.isInterState,
        isUnionTerritory: gst.isUnionTerritory,
        cgstAmount: gst.cgst,
        sgstAmount: gst.sgst,
        utgstAmount: gst.utgst,
        igstAmount: gst.igst,
      ...
      const uploaded = uploadBufferToLocal(pdfBuffer, {
        folder: 'invoices',
   ...
   */
  /**
   * Admin-recorded manual payment (cash/bank transfer/cheque) — same amount rules and
   * finalize path (invoice generation, order status, notifications) as a Razorpay payment;
   * only the gateway differs. Amount is always validated against the real outstanding
   * balance server-side — never trusts whatever figure the admin form sends beyond that.
   */
  async recordManual(
    orderId: bigint,
    ...
    if (order.status === 'cancelled') throw ApiError.badRequest('This order has been cancelled');
    if (order.paymentStatus === 'paid') throw ApiError.badRequest('This order is already fully paid');
    if (input.amount <= 0) throw ApiError.badRequest('Amount must be greater than zero');

    const totalAmount = Number(order.totalAmount);
    const alreadyPaid = await orderRepository.sumSuccessfulPayments(orderId);
    const balanceRemaining = Math.round((totalAmount - alreadyPaid) * 100) / 100;
    if (input.amount > balanceRemaining + 0.01) {
      throw ApiError.badRequest(`Amount exceeds the outstanding balance of ${balanceRemaining}`);
    }
