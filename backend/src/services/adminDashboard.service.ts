...
export class AdminDashboardService {
  async getSummary() {
    ...
    return {
      ...
      recentOrders,
      unreadAdminNotifications,
      totals: { schools: totalSchools, dealers: totalDealers, orders: totalOrders, quotations: totalQuotations },
    };
  }

  /** Schools ranked by real paid revenue (successful payments on their orders). */
  async getTopSchools(limit = 10) {
    const rows = await prisma.$queryRaw<Array<{ schoolId: string; schoolName: string; revenue: string | null; orderCount: bigint }>>`
      SELECT o.school_id AS schoolId, s.school_name AS schoolName, SUM(p.amount) AS revenue, COUNT(DISTINCT o.id) AS orderCount
      FROM payments p
      JOIN orders o ON o.id = p.order_id
      JOIN schools s ON s.id = o.school_id
      WHERE p.status = 'success'
      GROUP BY o.school_id, s.school_name
      ORDER BY revenue DESC
      LIMIT ${limit}
    `;
    return rows.map((r) => ({ schoolId: r.schoolId, schoolName: r.schoolName, revenue: Number(r.revenue ?? 0), orderCount: Number(r.orderCount) }));
  }
  ...
  /** Payment collections grouped by gateway — real, from actual successful payments. */
  async getPaymentMethodBreakdown() {
    ...
    return rows.map((r) => ({ method: r.gateway ?? 'unknown', amount: Number(r._sum.amount ?? 0), count: r._count._all }));
  }
}
