...
export class OrderService {
  /** Strips EduNest-internal procurement/margin fields before an order is ever returned to a school. */
private redactInternalFinancials<T extends Record<string, unknown>>(
  order: T,
): Omit<
  T,
  'dealerTotalAmount' | 'marginAmount' | 'platformFeeAmount' | 'netRevenueAmount'
> {
  const {
    dealerTotalAmount,
    marginAmount,
  ...
  void dealerTotalAmount;
  void marginAmount;
  void platformFeeAmount;
  void netRevenueAmount;

  return {
    ...rest,
    orderItems: Array.isArray(rest.orderItems)
      ? rest.orderItems.map((item: Record<string, unknown>) => {
          const { dealerUnitPrice, ...itemRest } = item;
          void dealerUnitPrice;
          return itemRest;
        })
      : rest.orderItems,
  } as Omit<
  ...
  async list(school: School, filters: { status?: OrderStatus; page?: number; limit?: number }) {
    ...
    return {
  items: items.map((o) => this.redactInternalFinancials(o)),
  ...
  async getById(school: School, id: bigint) {
    ...
    return this.redactInternalFinancials(order);
  }
