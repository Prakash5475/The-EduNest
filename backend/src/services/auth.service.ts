...
export class AuthService {
  async register(input: RegisterInput): Promise<{ user: User }> {
    ...
    void redis
      .set(emailVerifyKey(verifyToken), user.id.toString(), 'EX', EMAIL_VERIFY_TTL_HOURS * 3600)
      .then(() => emailService.sendWelcomeEmail(user.email, user.fullName, verifyToken))
      .catch((err) => {
        logger.error({ err, userId: user.id.toString() }, 'Registration verification setup failed');
      });
  ...
  /**
   * `identifier` is an email for admin/staff login, or — for dealer/school — the exact
   * business/school name used as their username per the login-by-name requirement.
   * `loginAs` narrows which lookup to use and doubles as a role check: a school name that
   * matches but was submitted via the Dealer tab is rejected, so the three login tabs can't
   * be used to fish for the other role's account.
   */
  async login(
    identifier: string,
    password: string,
    ...
    const user = await this.resolveLoginUser(identifier, loginAs);
  ...
  /** Resolves email/phone against users and enforces the selected login role. */
  private async resolveLoginUser(identifier: string, loginAs?: 'admin' | 'dealer' | 'customer'): Promise<User | null> {
    ...
    if (!user || !loginAs) return user;
    if (loginAs === 'customer') return user.userType === 'school' ? user : null;
    if (loginAs === 'dealer') return user.userType === 'dealer' ? user : null;
    return user.userType === 'admin' || user.userType === 'staff' ? user : null;
  }
