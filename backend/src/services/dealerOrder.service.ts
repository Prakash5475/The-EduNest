...
export class DealerOrderService {
  /** Order-level margin/revenue figures are EduNest-internal — a dealer only needs their own
   * per-item dealerUnitPrice (already on the order they're fulfilling), never the aggregate
   * margin/net-revenue/platform-fee EduNest makes on the sale. */
  private redactMarginFields<T extends Record<string, unknown>>(order: T) {
    const { marginAmount, netRevenueAmount, platformFeeAmount, dealerTotalAmount, ...rest } = order;
    void marginAmount; void netRevenueAmount; void platformFeeAmount; void dealerTotalAmount;
    return rest;
  }
  ...
  async list(dealer: Dealer, filters: { status?: OrderStatus; page?: number; limit?: number }) {
    ...
    return { items: items.map((o) => this.redactMarginFields(o)), meta: buildPaginationMeta(page, limit, total) };
  }

  async getById(dealer: Dealer, id: bigint) {
    const order = await orderRepository.findById(id);
    if (!order || order.dealerId !== dealer.id) throw ApiError.notFound('Order not found');
    return this.redactMarginFields(order);
  }
