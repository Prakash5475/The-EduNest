...
const preFeeTotal = subtotal + taxAmount + shippingAmount - discountAmount;
if (preFeeTotal <= 0) throw ApiError.badRequest('Order total must be greater than zero');

const dealerTotalAmount = 0;
const marginAmount = preFeeTotal;
const platformFeeAmount = (preFeeTotal * PLATFORM_FEE_PERCENT) / 100;
const netRevenueAmount = marginAmount + platformFeeAmount;
const totalAmount = preFeeTotal + platformFeeAmount;
