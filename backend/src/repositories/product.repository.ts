...
export class ProductRepository extends BaseRepository {
  async list(filters: ProductListFilters) {
    ...
    if (filters.inStockOnly) {
      // A product with NO inventory rows has never had stock configured — treat it as
      // available (not excluded), matching product.service.ts's computeAvailability(): only
      // a product with inventory rows that are all fully consumed should be filtered out.
      // Nested under AND (not OR) so this doesn't interfere with the search OR-clause above.
      where.AND = [
        ...(Array.isArray(where.AND) ? where.AND : where.AND ? [where.AND] : []),
        { OR: [{ inventory: { none: {} } }, { inventory: { some: { quantityAvailable: { gt: 0 } } } }] },
      ];
    }
