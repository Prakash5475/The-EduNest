...
export class QuotationRepository extends BaseRepository {
  ...
  createDealerQuotation(data: {
    ...
    validityDays: number;
    notes?: string;
    items: Array<{ quotationRequestProductId: bigint; schoolUnitPrice: number;
    dealerUnitPrice: number; quotedQuantity: number }>;
  }) {
    const totalAmount = data.items.reduce((sum, item) => sum + item.dealerUnitPrice * item.quotedQuantity, 0);
    return this.db.dealerQuotation.create({
      data: {
        ...
        totalAmount,
        status: 'submitted',
        dealerQuotationItems: {
          createMany: {
            data: data.items.map((item) => ({
              quotationRequestProductId: item.quotationRequestProductId,
              schoolUnitPrice: item.schoolUnitPrice,
              dealerUnitPrice: item.dealerUnitPrice,
              quotedQuantity: item.quotedQuantity,
              quotedUnitPriceLegacy: item.dealerUnitPrice,
            })),
          },
        },
  ...
  updateDealerQuotationItem(
  id: bigint,
  data: {
    schoolUnitPrice?: number;
