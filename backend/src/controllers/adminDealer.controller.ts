...
export class AdminDealerController {
  ...
  async update(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const dealer = await adminDealerService.updateDealer(BigInt(req.params.id), req.body);
      ApiResponse.success(res, { dealer }, 'Dealer updated successfully');
    } catch (err) {
      next(err);
    }
  }
  ...
  async create(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const dealer = await adminDealerService.createDealer(req.body);
      ApiResponse.created(res, { dealer }, 'Dealer created successfully');
    } catch (err) {
      next(err);
    }
  }
