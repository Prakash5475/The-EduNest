...
export class AdminDashboardController {
  ...
  async topSchools(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const limit = req.query.limit ? Number(req.query.limit) : undefined;
      const items = await adminDashboardService.getTopSchools(limit);
      ApiResponse.success(res, { items });
    } catch (err) {
      next(err);
    }
  ...
  async paymentMethods(_req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const items = await adminDashboardService.getPaymentMethodBreakdown();
      ApiResponse.success(res, { items });
    } catch (err) {
      next(err);
    }
