...
export class QuotationController {
  async create(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      let school;
      if (isStaff && req.body.schoolId) {
        school = await schoolRepository.findByIdWithProfile(BigInt(req.body.schoolId));
        if (!school) throw ApiError.notFound('School not found');
      } else {
        school = await requireSchoolContext(req.user);
      }
  ...
  unitPrice: Number(item.dealerUnitPrice),
  lineTotal: Number(item.dealerUnitPrice) * item.quotedQuantity,
})),
