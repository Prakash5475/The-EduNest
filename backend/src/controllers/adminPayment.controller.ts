...
export class AdminPaymentController {
  ...
  async recordManual(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const order = await paymentService.recordManual(BigInt(req.params.orderId), req.body);
      ApiResponse.created(res, { order }, 'Payment recorded');
    } catch (err) {
      next(err);
    }
  }
