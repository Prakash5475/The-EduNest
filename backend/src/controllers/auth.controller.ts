...
export class AuthController {
  ...
  async login(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { identifier, email, password, loginAs } = req.body;
      const device = getDeviceContext(req);
      const { user, accessToken, refreshToken, expiresIn } = await authService.login(identifier ?? email, password, device, loginAs);
