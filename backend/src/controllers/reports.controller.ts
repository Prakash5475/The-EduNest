...
export class ReportsController {
  ...
  async hsn(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const report = await reportsService.hsnReport(rangeFrom(req));
      const format = formatFrom(req);
      if (format) {
        await sendReportExport(res, format, { filename: 'hsn-report', title: 'HSN/SAC-wise GST Report', rows: report.rows });
        return;
      }
      ApiResponse.success(res, report);
    } catch (err) {
