...
export interface InvoicePdfData {
  ...
  subtotal: number;
  taxAmount: number;
  /** Exactly one of (cgst,sgst), (cgst,utgst), or (igst) is non-zero, per CBIC's intra-state / intra-UT / inter-state rule — never more than one pair. */
  isInterStateSupply: boolean;
  /** True for CGST+UTGST (administered Union Territories); false (even intra-state) means CGST+SGST. Ignored when isInterStateSupply is true. */
...
export function renderInvoicePdf(data: InvoicePdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(18).text(data.companyTradeName || 'The EduNest', { continued: true }).fontSize(10).text('  ');
    ...
    doc.moveDown(0.5);
    doc.fontSize(9).text(`Supplier: ${data.companyLegalName || data.companyTradeName || 'The EduNest'}`);
    if (data.companyGstin) doc.text(`Supplier GSTIN: ${data.companyGstin}`);
    if (data.companyAddress) doc.text(`Supplier Address: ${data.companyAddress}`);
    doc.text(`Place of Supply: ${data.isInterStateSupply ? 'Inter-State' : 'Intra-State'}`);
    doc.moveDown();
    ...
    y += 15;
    if (data.isInterStateSupply) {
      doc.text(`IGST: ${data.igstAmount.toFixed(2)}`, cols.total - 60, y);
      y += 15;
    } else if (data.isUnionTerritory) {
      doc.text(`CGST: ${data.cgstAmount.toFixed(2)}`, cols.total - 60, y);
      y += 15;
      doc.text(`UTGST: ${(data.utgstAmount ?? 0).toFixed(2)}`, cols.total - 60, y);
      y += 15;
    } else {
      doc.text(`CGST: ${data.cgstAmount.toFixed(2)}`, cols.total - 60, y);
      y += 15;
      doc.text(`SGST: ${data.sgstAmount.toFixed(2)}`, cols.total - 60, y);
      y += 15;
    }
    doc.text(
      `Platform / Razorpay Fee: ${data.platformFeeAmount.toFixed(2)}`,
      cols.total - 60,
      y,
    );
    y += 15;
