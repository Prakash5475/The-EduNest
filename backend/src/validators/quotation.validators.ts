...
const itemInput = z.object({
  ...
  customItemName: z.string().trim().max(220).optional(),
  customItemDescription: z.string().trim().max(255).optional(),
  customItemSchoolPrice: z.coerce.number().positive().optional(),
  customItemDealerPrice: z.coerce.number().positive().optional(),
  customItemImageFileId: z.coerce.bigint().optional(),
  customItemExternalUrl: z.string().trim().url().max(500).optional(),
  customItemNotes: z.string().trim().max(2000).optional(),
...
export const createQuotationRequestSchema = z.object({
  body: z.object({
    ...
    /** Admin/staff only — creates the request on behalf of this school instead of the caller's own. */
    ...
    items: z
      .array(
        z.object({
          ...
          dealerUnitPrice: z.coerce.number().positive().optional(),
