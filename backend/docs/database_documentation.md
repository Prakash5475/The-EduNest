...
**Verification:** the original schema (138 tables, 197 foreign keys) was executed end-to-end against a live MySQL-compatible server during authoring — every `CREATE TABLE`, constraint, and partition definition is confirmed syntactically valid, not just hand-written. **Phase 2** added 3 tables (141 total, 200 FKs) for the Product Customization workflow — see `CHANGELOG.md`; these follow the same conventions and passed the generator's structural self-check, but have not yet been re-run against a live server.
...
| 8B | Product Customization *(added Phase 2)* | customization_requests, customization_files, customization_status_history |
| 9 | Orders | orders (+shipping_method_id, Phase 3), order_items, order_status_history, order_timeline, order_notes, returns, cancellations, refunds, invoices (+invoice_type, Phase 3), invoice_items, shipments, tracking, delivery_updates, **shipping_methods** *(added Phase 3)* |
