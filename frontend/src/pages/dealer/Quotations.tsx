...
function QuoteEditor({ quotation, onClose }: { quotation: ApiDealerQuotation; onClose: () => void }) {
  ...
  const { register, handleSubmit } = useForm<Record<string, string>>({
    defaultValues: Object.fromEntries(quotation.dealerQuotationItems.map((i) => [i.id, i.dealerUnitPrice])),
  });
    ...
    mutationFn: (values: Record<string, string>) =>
      updateDealerQuotation(quotation.id, {
        items: quotation.dealerQuotationItems.map((i) => ({
          itemId: i.id,
          dealerUnitPrice: Number(values[i.id]),
        })),
      }),
    onSuccess: () => {
  ...
  const canEdit = quotation.status === "submitted";
