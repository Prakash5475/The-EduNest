...
export default function Settings() {
  ...
  useEffect(() => {
    settingsService.loadCompany().then(setCompany).catch(() => undefined);
    settingsService.loadGst().then(setGst).catch(() => undefined);
  }, []);
        ...
        <div className="space-y-6">
          <Card className="p-6">
            ...
            {active === "GST Settings" && (
              <>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <Field label="Legal Name (as per GST certificate)" value={gst.legalName} onChange={(v) => setGst({ ...gst, legalName: v })} />
                  <Field label="GSTIN" value={gst.gstNumber} onChange={(v) => setGst({ ...gst, gstNumber: v })} />
                  <Field label="Constitution" value={gst.constitution} onChange={(v) => setGst({ ...gst, constitution: v })} />
                  <Field label="Registration Type" value={gst.registrationType} onChange={(v) => setGst({ ...gst, registrationType: v })} />
                  <Field label="Registration Date" value={gst.registrationDate} onChange={(v) => setGst({ ...gst, registrationDate: v })} />
                  <Field label="Tax Registration State" value={gst.taxRegistrationState} onChange={(v) => setGst({ ...gst, taxRegistrationState: v, gstAddress: { ...gst.gstAddress, state: v } })} />
                  <Field label="Registered Address" value={gst.gstAddress.line1} onChange={(v) => setGst({ ...gst, gstAddress: { ...gst.gstAddress, line1: v } })} />
                  <Field label="City" value={gst.gstAddress.city} onChange={(v) => setGst({ ...gst, gstAddress: { ...gst.gstAddress, city: v } })} />
                  <Field label="Pincode" value={gst.gstAddress.pincode} onChange={(v) => setGst({ ...gst, gstAddress: { ...gst.gstAddress, pincode: v } })} />
                  <div className="space-y-1.5">
                    <Label>Default GST % (display only — CGST/SGST vs IGST is computed per-order from billing state)</Label>
