...
import { Plus, Download, Wallet, Clock, AlertTriangle, CalendarCheck, Undo2, Loader2 } from "lucide-react";
...
export default function Payments() {
  ...
  async function handleRefund() {
    ...
    setRefunding(true);
    try {
      await requestRefund(selected.id, selected.amount, "Requested by admin");
      toast.success(`Refund requested for ${selected.id}`);
    } catch (err) {
          ...
          {selected && (
            <>
              ...
              <div className="space-y-1.5 border-t border-border pt-4 text-sm">
                ...
                <Row label="Advance / Amount Paid" value={formatCurrency(selected.amount)} />
                <Row label="Remaining Balance" value={formatCurrency(Math.max(0, due))} />
              </div>

              {due > 0 && (
                <div className="rounded-xl bg-primary/5 p-4">
                  <p className="mb-1 text-sm font-semibold">Collect Remaining Balance</p>
                  <p className="mb-3 text-xs text-muted-foreground">
                    Prepared for Razorpay — will go live once the payment gateway is connected on the backend.
                  </p>
                  <RazorpayButton orderId={selected.orderId} amountType="balance" label={`Collect ${formatCurrency(due)} via Razorpay`} />
                </div>
              )}
