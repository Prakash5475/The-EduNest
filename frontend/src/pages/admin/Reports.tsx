...
export default function Reports() {
  ...
  const { data: gst, isLoading: gstLoading } = useQuery({
    ...
    queryFn: () => getGstReport(range),
  });
  const { data: hsn, isLoading: hsnLoading } = useQuery({
    queryKey: ["reports", "gst-hsn", range],
    queryFn: () => getHsnReport(range),
  });

  const isLoading = ordersLoading || paymentsLoading || quotationsLoading || invoicesLoading || gstLoading || hsnLoading;
  ...
  return (
    <div>
      ...
      <ChartCard title="GST Summary" description={`Company GST state: ${gst?.companyGstState || "not configured"} — backend-calculated, not editable from the frontend`} className="mt-6">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <SummaryRow label="Taxable Value" value={formatCurrency(gst?.totalTaxableValue ?? 0)} />
          <SummaryRow label="CGST" value={formatCurrency(gst?.totalCgst ?? 0)} />
          <SummaryRow label="SGST" value={formatCurrency(gst?.totalSgst ?? 0)} />
          <SummaryRow label="UTGST" value={formatCurrency(gst?.totalUtgst ?? 0)} />
          <SummaryRow label="IGST" value={formatCurrency(gst?.totalIgst ?? 0)} />
          <SummaryRow label="Total GST" value={formatCurrency(gst?.totalTaxCollected ?? 0)} valueClassName="text-primary" />
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                ...
                <th className="pb-2 text-right">UTGST</th>
                <th className="pb-2 text-right">IGST</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {(gst?.stateWiseSummary ?? []).length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-6 text-center text-muted-foreground">No taxable orders in this range.</td>
                </tr>
              ) : (
                (gst?.stateWiseSummary ?? []).map((row) => (
                  <tr key={row.state}>
                    <td className="py-2.5 font-medium">{row.state}</td>
                    <td className="py-2.5 text-right text-muted-foreground">{row.orderCount}</td>
                    <td className="py-2.5 text-right">{formatCurrency(row.taxableValue)}</td>
                    <td className="py-2.5 text-right">{row.cgst ? formatCurrency(row.cgst) : "—"}</td>
                    <td className="py-2.5 text-right">{row.sgst ? formatCurrency(row.sgst) : "—"}</td>
                    <td className="py-2.5 text-right">{row.utgst ? formatCurrency(row.utgst) : "—"}</td>
                    <td className="py-2.5 text-right">{row.igst ? formatCurrency(row.igst) : "—"}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          See the HSN/SAC-wise Report below for a per-code breakdown.
        </p>
        <div className="mt-4 flex justify-end">
          <ReportDownloadButtons
            title="GST State-wise Report"
            columns={GST_STATE_COLUMNS}
            rows={(gst?.stateWiseSummary ?? []).map((row) => ({ ...row }))}
            filenamePrefix="gst-report"
          />
        </div>
      </ChartCard>

      <ChartCard title="HSN/SAC-wise GST Report" description="Grouped by each product's assigned tax bracket (Settings > Taxes). Products with no tax bracket assigned show as 'Not assigned'." className="mt-6">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
            ...
            <tbody className="divide-y divide-border">
              {(hsn?.rows ?? []).length === 0 ? (
                <tr>
                  <td colSpan={10} className="py-6 text-center text-muted-foreground">No line items in this range.</td>
                </tr>
              ) : (
                (hsn?.rows ?? []).map((row) => (
                  <tr key={row.hsnCode}>
                    <td className="py-2.5 font-medium">{row.hsnCode}</td>
                    ...
                    <td className="py-2.5 text-right">{row.utgst ? formatCurrency(row.utgst) : "—"}</td>
                    <td className="py-2.5 text-right">{row.igst ? formatCurrency(row.igst) : "—"}</td>
                    <td className="py-2.5 text-right font-semibold">{formatCurrency(row.totalTax)}</td>
                    <td className="py-2.5 text-right text-muted-foreground">{row.invoiceCount}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Reflects each product's <em>current</em> tax bracket — if a product's HSN code changes after older orders exist, those orders regroup under the new code rather than the one charged at the time.
        </p>
        <div className="mt-4 flex justify-end">
          <ReportDownloadButtons
            title="HSN/SAC-wise GST Report"
            columns={HSN_COLUMNS}
            rows={(hsn?.rows ?? []).map((row) => ({ ...row }))}
            filenamePrefix="hsn-report"
          />
        </div>
      </ChartCard>
