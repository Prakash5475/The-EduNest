...
import { Plus, MoreVertical, Loader2, Tags, Layers } from "lucide-react";
import { CatalogManagerDialog } from "@/components/admin/CatalogManagerDialog";
...
export default function Products() {
  ...
  const [manageOpen, setManageOpen] = useState<"category" | "brand" | null>(null);
  ...
  function onSubmit(values: ProductFormValues) {
    ...
    const payload = {
      ...
      brandId: values.brandId && values.brandId.trim() !== "" ? values.brandId : undefined,
    };
    if (editingId) {
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" className="gap-2" onClick={() => setManageOpen("category")}>
              <Layers className="h-4 w-4" /> Manage Categories
            </Button>
            <Button variant="outline" className="gap-2" onClick={() => setManageOpen("brand")}>
              <Tags className="h-4 w-4" /> Manage Brands
            </Button>
      ...
      <CatalogManagerDialog
        kind={manageOpen ?? "category"}
        open={manageOpen !== null}
        onOpenChange={(open) => !open && setManageOpen(null)}
      />
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                {categories.length === 0 && (
                  <button
                    type="button"
                    onClick={() => setManageOpen("category")}
