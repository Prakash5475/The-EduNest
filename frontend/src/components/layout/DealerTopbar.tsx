...
export function DealerTopbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  const { data } = useQuery({
    queryKey: ["notifications", "dealer-topbar"],
    queryFn: () => listNotifications(1, 1),
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-2.5 rounded-xl py-1 pl-1 pr-2 hover:bg-muted focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            <Avatar className="h-9 w-9">
              <AvatarFallback>{initials(user?.fullName ?? "Dealer")}</AvatarFallback>
            </Avatar>
            <div className="hidden text-left leading-tight sm:block">
              <p className="max-w-[160px] truncate text-sm font-semibold">{user?.fullName ?? "Dealer"}</p>
