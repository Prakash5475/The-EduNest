...
import { loadCompanyProfile, type CompanyApiShape } from "@/services/settingsService";
...
export function PublicFooter() {
  const [company, setCompany] = useState<CompanyApiShape | null>(null);

  useEffect(() => {
    loadCompanyProfile().then(setCompany);
  }, []);
  ...
  return (
    <footer className="border-t border-border bg-edu-gray text-white">
      <div className="container grid grid-cols-1 gap-10 py-14 sm:grid-cols-2 lg:grid-cols-5">
        <div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-white/70">
            <li className="flex items-start gap-2.5">
              ...
              {addressText || "Baner, Pune, Maharashtra 411045"}
            </li>
            ...
            <li className="flex items-center gap-2.5">
              ...
              {company?.email || "hello@theedunest.com"}
            </li>
          </ul>
        </div>
      </div>
