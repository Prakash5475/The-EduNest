...
export interface GstStateSummaryRow {
  state: string;
  taxableValue: number;
  cgst: number;
  sgst: number;
...
export interface GstReport {
  ...
  stateWiseSummary: GstStateSummaryRow[];
...
export async function getHsnReport(range: DateRange = {}): Promise<HsnReport> {
  return apiClient.get<HsnReport>("/reports/gst/hsn", { query: range as Record<string, string> });
}
