...
function resolveMediaUrl(filePath: string): string {
  if (/^https?:\/\//i.test(filePath) || !filePath.startsWith('/')) return filePath;
  if (!/^https?:\/\//i.test(API_BASE_URL)) return filePath;
  return `${new URL(API_BASE_URL).origin}${filePath}`;
}
...
interface ApiProduct {
  ...
  availability?: { quantityAvailable: number; inStock: boolean; isTracked?: boolean; stockStatus?: "not_tracked" | "out_of_stock" | "low_stock" | "in_stock" };
}
...
function deriveStockStatus(product: ApiProduct): StockStatus {
  // Trust the backend's computed status when present — it correctly distinguishes "stock
  // was never configured" (not_tracked) from "configured and genuinely zero"
  // (out_of_stock). Bug fix: previously, missing/empty inventory data fell through this
  // function's own reduce (which defaults to 0) and always rendered as Out of Stock, even
  // when no stock had been entered for the product at all.
  if (product.availability?.stockStatus) {
    switch (product.availability.stockStatus) {
      case "not_tracked":
      case "in_stock":
        return "in-stock";
      case "low_stock":
  ...
  // Fallback for any endpoint that doesn't send `availability` at all (e.g. a list view
  // using a lighter DTO): no inventory rows means stock was never configured, not that it's
  // zero — never default that case to "out-of-stock".
...
export function adaptProduct(p: ApiProduct): Product {
  const images = (p.productImages ?? [])
    .map((img) => (img.uploadedFile?.filePath ? resolveMediaUrl(img.uploadedFile.filePath) : undefined))
...
export async function listBrandsForForm(): Promise<BrandOption[]> {
  const { data } = await apiClient.withMeta<ApiBrand[]>("/brands", { query: { limit: 100 }, anonymous: true });
